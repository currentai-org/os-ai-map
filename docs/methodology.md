...
The exact cutoffs (four category-leading fully-open products for Stage 5, the 4.5 category-leading bar, and best-fully-open score bands of 3.5, 3.0, and 2.0 dividing the lower stages) are deliberate, tunable parameters, reviewed when the scoring rubric or curation density changes materially. Not every category needs to reach Stage 5; redundancy matters more in some parts of the stack than others.
...
Each gap names a different kind of shortfall, so which one fires follows from the stage. At Stage 4 a category has proven category-leading open options but not enough of them, so its only stage-derived gap is **resiliency**; a declared **disclosure** may coexist. Below Stage 4 nothing fully open clears the bar, so we name the drivers that hold the best fully open option back — capability, adoption, or both — rather than restating the stage. Both can fire together, and neither is required. Where the best fully open option clears both cutoffs while the blend still misses the bar, that reading has nothing to say, and we ask a wider one instead: has any fully open product in the category topped the axis at all? An axis nothing has topped is named, which is how a category whose best option sits at 4 and 4 still gets told what is holding it back. The same two readings handle a missing grade: some product types have no capability signal worth grading, their capability grade is null rather than low, and a null cannot be below a cutoff — but the wider question is still asked of the category's other open products, so a capability gap can still fire there. Only where both readings are silent does a category sit below Stage 4 with no gap named, and that happens two ways: an axis no fully open product has graded at all, or two axes both topped but never by the same product. Neither is a category with nothing missing, and the empty set does not say which of the two it is.
