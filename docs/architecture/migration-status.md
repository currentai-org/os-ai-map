...
| 5 — catalog split and long-tail migration | in progress | <!-- count:pending -->2 assets carry `migration_status: pending` (`stack_map`, `pypi_downloads`). **Scope decision (2026-08-29):** `catalog.osai_gap_map`, `catalog.osai_subcategory_mapping` and `catalog.taxonomy_crosswalk` were **removed from the inventory as out of repo scope** — platform-authored reference tables read only by the platform-authored, off-release-path `scores.taxonomy` / `scores.investment_ranking` (which were themselves removed for the same reason). The repo tracks the CORE gap-map datasets + reference-notebook inputs, not a mirror of the OSO org; these live on OSO and are discoverable in its UI. This is an inventory-only removal — the org-wide audit receipt still records the deployed island models as consumers of the in-scope tables they read (retirement-safety data), so core tables keep their `platform_model_consumers` edges. That retired the two planned `osai_subcategory_mapping` / `taxonomy_crosswalk` registry transitions entirely. See data-architecture.md §11.3 (What the inventory excludes); guarded by `test_out_of_scope_platform_tables_are_not_tracked`. **Dataset-type correction (2026-08-28, #393):** the planned `entities.*→catalog` (4) and `scores.*→evaluation` (8) relocations are **cancelled** and those 12 scheduled models are now `migration_status: not_planned` (`target_namespace = current_namespace`, `not_planned_reason` recording the constraint), which drops `pending` 19→7. The reason is physical, not a sequencing choice: an OSO namespace **is** a dataset, its type (`STATIC_MODEL` vs `USER_MODEL`) is **fixed at creation and immutable**, and the two target namespaces are static — `currentai.catalog` is `STATIC_MODEL` (`046ee25e`) and `currentai.evaluation` is the static compiled release-artifact dataset (`55a4311d`: `axis_*`, `adoption_*`) — while `entities.*` (`663132ed`, cron `0 4 * * 0`) and `scores.*` (`48ddf155`, cron `0 4 * * 1`) are scheduled `USER_MODEL` pipelines. A scheduled model cannot be hosted in a static dataset; **platform-verified on the catalog case 2026-08-28** — a create/revision/release into `catalog` succeeded silently but the run request was rejected because the dataset type is `STATIC_MODEL` where a scheduled model needs `USER_MODEL` (the created model was deleted; catalog left untouched). Decision: the scheduled discovery pipeline stays in its own `USER_MODEL` namespace `entities` and the scoring pipeline stays in `scores`; `catalog` and `evaluation` remain the static layers they already are. The `kind` fields (`catalog` / `evaluation`) are unchanged — the logical model is intact; this is the `not_planned` exception to `target_namespace == kind`. **The two rename collisions are dropped** (`catalog.model_benchmarks→openllm_leaderboard`, `catalog.model_repos→hf_model_repo_links`): their only trigger was the `catalog.models` name collision the `entities→catalog` move would have created, which now never occurs, and §11.6 forbids a PR that exists purely to rename a deployed table. **Execution plan:** `docs/operations/phase5-namespace-migration.md`; per-move steps in `docs/operations/phase5-execution-runbook.md`. **Scheduling correction (2026-08-28, #393):** the planned `events.github_events` + `metrics.daily` → `observations` fold is **deferred** and both are now `not_planned` (kept in their own datasets), dropping `pending` 7→5. An OSO schedule is a **dataset-level sweep**; a model cron only throttles it (platform-verified 2026-08-28). `observations` is currently manual (`cron null` — `product_adoption_current` runs `MANUAL` under the §18 baseline discipline), and `events` (`0 5 * * 0`) / `metrics` (`0 6 * * 0`) carry distinct dataset crons. Folding them in would force a sweep cron on the observations dataset that also swept `product_adoption_current` and the pypi orphan — a behavior change to a §18-sensitive model — so both are `not_planned` and stay in their own datasets. Whether `observations` ever gains a sweep is a Phase-2B refresh-model decision (2B owns it); a later relocation of `events`/`metrics` would need a new explicit decision, not an automatic follow-on. **The remaining pending after the scope decision:** `catalog.stack_map` (a real `catalog.*→registry` ownership transition — repo-authored, feeds a reference notebook + a repo model, so genuinely in scope; disposition: move to `registry.stack_map`, not retire-into-`product_scores` — that assumption was wrong, product_scores is product-grain and lacks the repo→taxonomy mapping stack_map's consumers join on) and `catalog.pypi_downloads` (**deferred** — notebook-only, no scoring impact — see the Phase-5 plan's Freshness section). **`catalog.foundation_model_repos → registry` — consumer cutover underway:** its data now lives at `sources/foundation_model_repos.yaml`, compiled by `serialize_registry` into `registry.foundation_model_repos`. The create-target landed in **#397**; `registry.yml` **published it on merge** and it is **platform-verified 2026-08-29** — 72 rows, 72 distinct `model_family`, and a bidirectional business-column diff against the retiring catalog table is clean (`only_in_catalog` 0, `only_in_registry` 0), so `registry.foundation_model_repos` is now `status: active` / `materialized: true`. The **reconciliation PR** repoints the in-repo consumer `entities.models` from `catalog.foundation_model_repos` to the registry table (SQL + inventory `reads` + DAG), so the catalog table has no in-repo reader left. `catalog.foundation_model_repos` stays live (`in_progress`) **only** because the deployed `state_of_os_ai.family_footprint` still reads it; it retires under §17 once that deployed reader is repointed (a separately authorized maintainer/platform step). **§17 parity caveat:** both tables carry regenerated dlt loader columns (`_dlt_load_id`, `_dlt_id`), so a full-row `EXCEPT` reports total drift permanently — the retirement equivalence check MUST project the business columns explicitly. That leaves **one** transition still to author — `catalog.stack_map → registry.stack_map` (the planned `osai_subcategory_mapping` / `taxonomy_crosswalk` transitions were retired by the 2026-08-29 scope decision above). **Emerging pattern across #393:** OSO binds one type and one sweep schedule per dataset, so independently-typed or independently-scheduled pipelines (`entities`, `scores`, `events`, `metrics`) cannot fold into a shared namespace; Phase 5's real remaining work is the compiled `catalog.*→registry` transitions. **Separately flagged (repo↔platform drift, needs handling):** the platform agent found the deployed `events.github_events` (rev 8) and `metrics.daily` (rev 5) read `oso.github_events.github_events_last_365_days`, while the repo SQL + inventory still read the older `oso.int_events__github_unified` / `oso.*opendevdata*` (repo file sha256 ≠ the Phase-0b audit's deployed `source_sha256` for both) — a pre-existing `authority: repo` breach independent of the move, **resolved 2026-08-29 (#395)**: the deployed rev-8/rev-5 source was back-ported verbatim, and the repo file sha256 now matches the Phase-0b audit's deployed `source_sha256` for both (`9fcefe…`, `5941d4…`). Known follow-up: the back-ported prose still carries a stale claim of a frozen-history union the SQL no longer performs. **The earlier six inventory corrections landed 2026-08-28** (#392): the three `signal_*.product_adoption` tables and the three Phase-7 openness-chain tables (`scores.openness_facts`, `scores.openness_computed`, `evidence.product_evidence`) are `migration_status: not_planned` with `target_namespace = current_namespace` — they do **NOT** relocate to `evaluation`. Distinct lifecycle preserved: `signal_github`/`signal_huggingface` → `status: compatibility` naming `observations.product_adoption_current` as `replacement` (re-read live 2026-08-28: 140 / 115 rows; `signal_github` left the retirement-candidate and no-reviewed-consumer lists, both 8→7); `signal_packages` stays `status: staged` (never deployed, #314); the openness chain stays `status: active` until its Phase 7 / §9.3 retirement (#384). Guarded by `test_not_planned_assets_stay_put_with_a_documented_disposition`, `test_signal_product_adoption_tables_stay_put_and_deployed_ones_are_compat`, and `test_phase7_openness_chain_stays_put_until_retirement`. **The three `catalog.*→registry` moves are ownership transitions** (registry compiles from `sources/`; `models/registry/` is forbidden), needing a canonical source format + serializer integration + registry-publisher path, not a SQL repoint; every move follows the **source-preserving lockstep** (create target alongside source → verify → repoint consumers while source is live → reconcile the repo → retire later under separate authorization), with a **scheduling gate** requiring a successful scheduled run on the target before canonical consumers are repointed for scheduled moves into `observations`. **Scope correction (2026-08-26):** a pilot that relocated `signal_semanticscholar.paper_citations` to `observations` was **rolled back** — §4.3 keeps source-specific ingestion in `signal_*` (a `kind: observations` table in a `signal_*` namespace is permanently legitimate), and a regression gate (`test_source_collectors_are_not_reclassified_to_observations`) now protects source collectors from that reclassification. The 429 hardening from the aborted pilot was kept in place on the `signal_semanticscholar` collector. |
...
**Retirement candidates: <!-- count:retirement_candidates -->7.** Phase 0b read all 41 deployed
...
The <!-- count:no_reviewed_consumer -->7 assets below are **deployed** and have **no reviewed
