...
| 5 — catalog split and long-tail migration | in progress | **Pilot move executed** (2026-08-26, `docs/operations/catalog-split.md` Instance 1): `signal_semanticscholar.paper_citations` relocated to `observations.paper_citations`. The twin deployed with code byte-identical except the sanctioned `capabilities=oso.Capabilities(fetch=True)` declaration — the runbook's Instance 1 predicted no capabilities delta, but the model does call `context.fetch()` and the release gate refused the byte-identical bytes, so the general procedure's one sanctioned delta applied. Both in-repo consumers repointed (`product_adoption_current` FROM + `source_dataset`/`source_table` literals; `signal_routing.yaml` `semanticscholar.table`, no `routing_policy_version` bump). The old table stays deployed as a `compatibility` asset until step-6 retirement. Both models still fail their live fetch on the shared upstream Semantic Scholar `429` — proven shared at paired run times 2026-08-26, identical error text either side. The `observations` dataset has **no cron**, so the relocated fetcher runs manually until that dataset is scheduled — a recorded follow-up, not a defect of the move. The twin **first materialized 2026-08-26 05:57Z**, on the tenth attempt that day after nine consecutive shared 429s — 24 rows, all found, pairing 1:1 with the old table on (product_slug, arxiv_id) with identical paper identity and non-decreasing citation counts (+1,148 over the ten days between fetches). The 429 fix landed the same day as its own change, after the move completed on identical bytes: revisions 5-6 of the relocated model authenticate with the `SEMANTIC_SCHOLAR_TOKEN` platform secret (x-api-key), pace batch POSTs to the keyed one-request-per-second allowance, and retry load-shed 429s with exponential backoff (15s doubling to 120s), verified with SUCCESS runs 06:31Z and 07:22Z. The old compatibility model stays anonymous and un-retried — it is retiring at step 6, not being maintained. **Step 3's platform redeploy is done**: `product_adoption_current` released as revision 4 (`3da509bb`, deployed from the repointed repo SQL, source sha `c4b98994…` matching the repository byte for byte) and re-materialized cleanly under the declared-identity guard 2026-08-26 06:03Z — 654 rows, per-channel counts unchanged (github 305, huggingface 189, pypi 136, arxiv 24), the citations channel now carrying `source_table = currentai.observations.paper_citations`. <!-- count:pending -->36 assets carry `migration_status: pending`. |
