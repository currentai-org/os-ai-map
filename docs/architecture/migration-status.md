...
| 5 — catalog split and long-tail migration | in progress (planning) | <!-- count:pending -->25 assets carry `migration_status: pending`. **Execution plan written 2026-08-28** (`docs/operations/phase5-namespace-migration.md`, PLAN FOR REVIEW — no asset moved, no platform mutation): the 25 reconcile as **16 sanctioned relocations + 6 corrections/out-of-phase retirement targets + 3 nuanced-sequencing cases**; the two rename collisions (`catalog.model_benchmarks→openllm_leaderboard`, `catalog.model_repos→hf_model_repo_links`) ride the `entities→catalog` move. **Correction findings (same class as the semanticscholar rollback below):** the three `signal_*.product_adoption` rows should become in-place **compatibility** assets (`replacement: observations.product_adoption_current`), not relocations to `evaluation`; and `scores.openness_facts`, `scores.openness_computed`, and `evidence.product_evidence` carry `target_namespace: evaluation` but are **Phase 7 / §9.3 retirement targets** (replaced by `evaluation.axis_*`, #384), not Phase 5 moves. These corrections land first, inventory-only with a regression gate. **First corrections landed 2026-08-28** (inventory-only, no platform mutation): the two DEPLOYED signal tables (`signal_github.product_adoption`, `signal_huggingface.product_adoption`) are now `status: compatibility` naming `observations.product_adoption_current` as their `replacement`, guarded by `test_signal_product_adoption_tables_are_superseded_compatibility_shims`; `signal_github` thereby left the derived retirement-candidate and no-reviewed-consumer lists (both 8→7). **Deferred with cause, NOT applied:** `signal_packages.product_adoption` is `staged`/never-deployed (#314), so it is not a live compatibility shim and is exempt; and `scores.openness_facts`/`openness_computed` + `evidence.product_evidence` are §9.3 retirement targets whose retirement is gated on multi-release dual-run acceptance (not yet met), so marking them `compatibility` now would be premature — their Phase-5 exclusion is the planning classification above, with no inventory change yet. A residual architecture note: the merged source-collector gate's docstring calls these tables movers that "legitimately move to evaluation," which reads against §11.1's retire-in-place; the compatibility reclassification follows §11.1 without disturbing that gate's enforcement. **Two execution subtleties the plan pins:** the three `catalog.*→registry` moves are **ownership transitions** (registry compiles from `sources/`; `models/registry/` is forbidden), needing a canonical source format + serializer integration + registry-publisher path, not a SQL repoint; and every move follows the **source-preserving lockstep** (create target alongside source → verify → repoint consumers while source is live → reconcile the repo → retire later under separate authorization), with a **scheduling gate** requiring a successful scheduled run on the target before canonical consumers are repointed for moves into `observations` and scheduled models into `evaluation`. **Scope correction (2026-08-26):** a pilot that relocated `signal_semanticscholar.paper_citations` to `observations` was **rolled back** — `data-architecture.md` §4.3 keeps source-specific ingestion in `signal_*` (a `kind: observations` table in a `signal_*` namespace is permanently legitimate), and §11.6 only authorizes `entities→catalog`, `events`/`metrics→observations`, `scores→evaluation`. The `target_namespace: observations` values on the `signal_*` rows were misfiled and **have been corrected** to stay put (`target_namespace = current_namespace`, `migration_status: complete`); a regression gate (`test_source_collectors_are_not_reclassified_to_observations`) now protects source collectors from that reclassification. Phase 5's real scope is the sanctioned moves — `entities→catalog`, `events`/`metrics`/`catalog.pypi_downloads→observations`, the curator-controlled `catalog.*→registry` ownership transitions, and the retained analytical `scores.*→evaluation` set (`dependency_graph`, `fragility`, `ossd_coverage`, `project_summary`, `repos_summary`, `stack_contributors`). `evidence.product_evidence` and `scores.openness_facts`/`openness_computed` are NOT in that scope — they retire in Phase 7 / §9.3. The 429 hardening from the aborted pilot was kept in place on the `signal_semanticscholar` collector. |
