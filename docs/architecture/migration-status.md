...
| 5 — catalog split and long-tail migration | not started | <!-- count:pending -->35 assets carry `migration_status: pending`. **Scope correction (2026-08-26):** a pilot that relocated `signal_semanticscholar.paper_citations` to `observations` was **rolled back** — `data-architecture.md` §4.3 keeps source-specific ingestion in `signal_*` (a `kind: observations` table in a `signal_*` namespace is permanently legitimate), and §11.6 only authorizes `entities→catalog`, `events`/`metrics→observations`, `scores→evaluation`. The `target_namespace: observations` values on the remaining `signal_*` rows are misfiled and will be corrected to stay put; Phase 5's real scope is the sanctioned moves plus the `scores`/`evidence → evaluation` cluster (held for Phase 6). The 429 hardening from the aborted pilot was kept in place on the `signal_semanticscholar` collector. |
