...
| 2 — normalized adoption observations | in progress | **2A done**: `registry.adoption_routes` + `adoption_route_scopes` / `adoption_route_band_sets` / `adoption_aggregation_rules` compiled from `signal_routing.yaml` and merged (#351), materialized on the platform 2026-08-23 (7 / 1 / 20 / 1 rows). **`source_runs` shipped** (#356). **`artifact_state` source-table rename applied** (Part B runbook): `signal_github.repo_state` and `signal_huggingface.hub_state` twinned to `signal_github.artifact_state` / `signal_huggingface.artifact_state` on the platform, the three consumers repointed and redeployed, and the old tables kept as `compatibility` assets (each naming its replacement) until their step-6 retirement. **`observations.product_adoption_current` deployed** (2026-08-24) — the artifact-level, band-free current-state normalization over the four deployed adoption sources; the deploy created the `observations` namespace, and the first run materialized 654 rows over 392 products (`authority: repo`, deployed from the repo SQL). The `product_adoption_baseline` **is** a Phase 2 item: §18 requires one preserved baseline snapshot for the temporary full-refresh period, and because `product_adoption_current` is full-refresh (every run overwrites), the deployed 654-row state is preserved only if frozen now — so the current bytes are captured as the immutable, UTC-timestamped baseline at `warehouse/data/observations/product_adoption_baseline.parquet`, recording the honest absence of authoritative row-to-run binding (#355). Phase 2 is **not** complete until that baseline is captured. The per-source banding roll-ups retire later, once the evaluation layer + reconciliation replace them. |
| 2B — incremental adoption history | blocked | Blocked on OSO incremental-model support (issue #352). The stable latest-state contract `observations.product_adoption_current` (full-refresh + `observed_at`) ships in Phase 2; the bare name `observations.product_adoption` is reserved for the incremental append-only history table, created only when OSO supports it. The `observations.product_adoption_baseline` frozen-bytes snapshot is captured in **Phase 2**, not here — it is the immutable anchor the append-only history builds **forward** from, so it must already exist when 2B lands. `_current` is not renamed into `product_adoption` — its grain is current-state, and a consumer of the current-state table must not silently inherit a historical grain. |
