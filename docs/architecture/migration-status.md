...
**The adoption observation → reconciliation thread (Phases 2–4) ships complete without run-id
binding or incremental history.** Row-to-run binding (#355), the Phase-4 blocking gate, and the
incremental append-only history (Phase 2B) are **follow-up work**, bundled with OSO's
incremental-model support (issue #352; the run id the binding needs is folded in as Kariba
OSO-4705). The shipped end state is the report — `evaluation.adoption_reconciliation` with every
measured row `source_unavailable` — which is sound to publish and unsound to gate on, exactly as
§4.3 requires. Phases 5–8 remain separate in-scope tracks, unaffected by this.
...
next phase's. The Phase-4 blocking gate is the reader, and it is **follow-up work** (see the phase
table): it stays disabled while every measured reconciliation row is `source_unavailable` for want
of row-to-run binding (#355), and that binding is itself follow-up — the platform run id it needs is
folded into OSO's incremental-model work (Kariba OSO-4705). So `source_unavailable` is the shipped
end state of the reconciliation report, not a temporary one waiting on this migration. Publishing the
