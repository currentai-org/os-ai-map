...
| 5 — catalog split and long-tail migration | in progress | <!-- count:pending -->36 assets carry `migration_status: pending`. The general move procedure is documented in `docs/operations/catalog-split.md` (it generalizes the Phase-2 `artifact-state-rename.md`); each coordinated move is a maintainer platform flip plus a lockstep repository PR. **No move is applied yet** — the runbook is docs-only. **Instance 1 is defined**: `signal_semanticscholar.paper_citations → observations.paper_citations`, the smallest fully-isolatable move (one in-repo consumer, `observations.product_adoption_current`; no external or platform consumers; no entanglement with the held cluster). The `scores.*`/`evidence.* → evaluation` cluster (the ten `scores.*` tables plus `evidence.product_evidence`) is **held** until Phase 6's repository-owned scoring trace lands, since Phase 6 replaces and Phase 7 retires exactly those tables. The two `status: compatibility` rows `signal_github.repo_state` / `signal_huggingface.hub_state` are `artifact-state-rename.md` step-6 **retirements**, not Phase-5 moves. |
