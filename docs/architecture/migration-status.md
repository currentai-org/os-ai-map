...
| 2 — normalized adoption observations | in progress | **2A done**: `registry.adoption_routes` + `adoption_route_scopes` / `adoption_route_band_sets` / `adoption_aggregation_rules` compiled from `signal_routing.yaml` and merged (#351), materialized on the platform 2026-08-23 (7 / 1 / 20 / 1 rows), so the routing precedence is no longer repo-only. Remaining: the `observations.*` layer (`source_runs`, `product_adoption`, baseline) before any signal roll-up retires. |
| 2B — incremental adoption history | blocked | Blocked on OSO incremental-model support (issue #352). `observations.product_adoption` ships first as a full-refresh model carrying `observed_at` — a snapshot, not retained history — and migrates full → incremental when OSO supports it. |
