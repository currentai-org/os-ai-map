...
| 5 — catalog split and long-tail migration | in progress (planning) | <!-- count:pending -->25 assets carry `migration_status: pending`. **Execution plan written 2026-08-28** (`docs/operations/phase5-namespace-migration.md`, PLAN FOR REVIEW — no asset moved, no platform mutation): the 25 pending assets are classified into 16 sanctioned relocations, 5 requiring a `target_namespace`/`status` correction **before** any move, and 4 nuanced-sequencing cases; the two rename collisions (`catalog.model_benchmarks→openllm_leaderboard`, `catalog.model_repos→hf_model_repo_links`) ride the `entities→catalog` move. **Correction findings (same class as the semanticscholar rollback below):** the three `signal_*.product_adoption` rows carry `target_namespace: evaluation`, but §11.1 makes them transitional **compatibility** assets (`replacement: observations.product_adoption_current`) retired in place, not relocations; and `scores.openness_facts` / `scores.openness_computed` carry `target_namespace: evaluation` but are **Phase 7 retirement targets** (replaced by `evaluation.axis_*`, #384), not Phase 5 moves. These corrections land first, as an inventory-only unit with a regression gate. **Scope correction (2026-08-26):** a pilot that relocated `signal_semanticscholar.paper_citations` to `observations` was **rolled back** — `data-architecture.md` §4.3 keeps source-specific ingestion in `signal_*` (a `kind: observations` table in a `signal_*` namespace is permanently legitimate), and §11.6 only authorizes `entities→catalog`, `events`/`metrics→observations`, `scores→evaluation`. The `target_namespace: observations` values on the `signal_*` rows were misfiled and **have been corrected** to stay put (`target_namespace = current_namespace`, `migration_status: complete`); a regression gate (`test_source_collectors_are_not_reclassified_to_observations`) now protects source collectors from that reclassification. Phase 5's real scope is the sanctioned moves — `entities→catalog`, `events`/`metrics→observations`, and the `scores`/`evidence → evaluation` cluster (held for Phase 6). The 429 hardening from the aborted pilot was kept in place on the `signal_semanticscholar` collector. |
