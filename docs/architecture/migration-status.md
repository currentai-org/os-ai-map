...
| 5 — catalog split and long-tail migration | in progress | <!-- count:pending -->19 assets carry `migration_status: pending` — **16 sanctioned relocations + 3 nuanced-sequencing cases**; the two rename collisions (`catalog.model_benchmarks→openllm_leaderboard`, `catalog.model_repos→hf_model_repo_links`) ride the `entities→catalog` move. **Execution plan:** `docs/operations/phase5-namespace-migration.md` (PLAN FOR REVIEW). **The six inventory corrections landed 2026-08-28** (#392, inventory-only, no platform mutation): the three `signal_*.product_adoption` tables and the three Phase-7 openness-chain tables (`scores.openness_facts`, `scores.openness_computed`, `evidence.product_evidence`) are now `migration_status: not_planned` with `target_namespace = current_namespace` — they do **NOT** relocate to `evaluation` (the misfiled relocation claim is removed, not merely annotated), which drops `pending` 25→19. Distinct lifecycle preserved per asset: `signal_github`/`signal_huggingface` → `status: compatibility` naming `observations.product_adoption_current` as `replacement` (both re-read live 2026-08-28: 140 / 115 rows materialized; `signal_github` thereby left the retirement-candidate and no-reviewed-consumer lists, both 8→7); `signal_packages` stays `status: staged` (never deployed, #314); the openness chain stays `status: active` until its Phase 7 / §9.3 retirement (#384, gated on multi-release dual-run agreement). Guarded by `test_not_planned_assets_stay_put_with_a_documented_disposition`, `test_signal_product_adoption_tables_stay_put_and_deployed_ones_are_compat`, and `test_phase7_openness_chain_stays_put_until_retirement`; the stale source-collector docstring claim that these "legitimately move to evaluation" was removed. **Two execution subtleties the plan pins:** the three `catalog.*→registry` moves are **ownership transitions** (registry compiles from `sources/`; `models/registry/` is forbidden), needing a canonical source format + serializer integration + registry-publisher path, not a SQL repoint; and every move follows the **source-preserving lockstep** (create target alongside source → verify → repoint consumers while source is live → reconcile the repo → retire later under separate authorization), with a **scheduling gate** requiring a successful scheduled run on the target before canonical consumers are repointed for moves into `observations` and scheduled models into `evaluation`. **Scope correction (2026-08-26):** a pilot that relocated `signal_semanticscholar.paper_citations` to `observations` was **rolled back** — `data-architecture.md` §4.3 keeps source-specific ingestion in `signal_*` (a `kind: observations` table in a `signal_*` namespace is permanently legitimate), and §11.6 only authorizes `entities→catalog`, `events`/`metrics→observations`, `scores→evaluation`. The `target_namespace: observations` values on the `signal_*` rows were misfiled and **have been corrected** to stay put (`target_namespace = current_namespace`, `migration_status: complete`); a regression gate (`test_source_collectors_are_not_reclassified_to_observations`) now protects source collectors from that reclassification. Phase 5's real scope is the sanctioned moves — `entities→catalog`, `events`/`metrics`/`catalog.pypi_downloads→observations`, the curator-controlled `catalog.*→registry` ownership transitions, and the retained analytical `scores.*→evaluation` set (`dependency_graph`, `fragility`, `ossd_coverage`, `project_summary`, `repos_summary`, `stack_contributors`). `evidence.product_evidence` and `scores.openness_facts`/`openness_computed` are NOT in that scope — they retire in Phase 7 / §9.3. The 429 hardening from the aborted pilot was kept in place on the `signal_semanticscholar` collector. |
