...
| 3 — reconciliation report | in progress | **Both fully-keyed evaluation tables built as repo-side release builders**, keyed on `declaration_version_id` + `observation_snapshot_id`. **`evaluation.product_adoption_measurements`** (`build/adoption_measurements.py`) aggregates the artifact-level `observations.product_adoption_current` to the product level: route selection by `registry.adoption_routes` precedence + `adoption_route_scopes`, aggregation by `adoption_aggregation_rules` (`usage_volume` sums across a family's artifacts), banding by `adoption_route_band_sets` → `adoption_bands`. It reinterprets none of that — the routing compiler (`build/serialize_routing.py`) is the one owner. A rule-less route with several contributing artifacts (only `stars_fallback` reaches this — 3 products with multiple GitHub repos in the baseline) **abstains** rather than invent a sum/max the source never declared; declaring a stars aggregation rule would resolve it. **`evaluation.adoption_reconciliation`** (`build/adoption_reconciliation.py`) compares measured vs recorded adoption, **report-first** (assigns status + explanation, blocks nothing). Every measured row is `source_unavailable` today because observations carry no `source_run_id` (row-to-run binding blocked on #355), exactly as §4.3 requires — the fuller status set activates once #355 binds observations to runs. Both are pure functions of their inputs with the identities passed in, exercised in-repo against the immutable Phase-2 baseline with pinned goldens (`tests/test_adoption_evaluation.py`). **Not deployed**: the `evaluation` namespace does not exist on the platform yet, both are inventoried `staged` / `materialized: false`, and publishing to OSO is a maintainer step. |
