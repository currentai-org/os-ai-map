...
new org whose product declares a `github` or `huggingface_*` artifact raises the denominator and
drops coverage below its pinned baseline. None of the four gates below sees it — it fails in CI,
under `validate`, as `test_the_live_corpus_is_at_or_above_the_committed_baseline`. Add one row per
`{org, platform, handle}`; `uv run python -m build.propose_org_handles --out <path>` will confirm
the `huggingface` namespace is unambiguous. The handle is the ACCOUNT name and need not match the
org slug (`fair-forward` publishes code under `datenlabor-bmz`); where it does not, say why in a
`note`. See [`../reference/identity.md`](../reference/identity.md#handles-are-ownership-evidence-not-adoption-evidence).
...
The review sheet generated by `build/check_corpus_diff` on your PR compares the committed
sources to the changes your work makes. Commit your changes before opening the PR to ensure the
sheet accurately reports which products you touched and detects any silent rewrites in untouched
products.
