...
is deterministic. Do **not** claim byte-for-byte data (row) parity: two independently running
external fetchers cannot guarantee identical rows, because live counts and timestamps move
between requests, so a per-row comparison of two separately-fetched tables would trip on drift
...
- **Identical schema and identity coverage** — column names, types and nullability identical
  ...
  present in both, with no dropped or invented keys.
- **A documented comparison of the dynamic fields** — counts, `fetched_at`, and the other fields
  that move between fetches, captured at paired run times, with the expected drift explained. A
  divergence in these fields is not by itself a failure; an unexplained one is.

Only assert exact data equality if **both** models are run against the same captured input
snapshot; otherwise do not advertise it. If code, schema or identity coverage diverges, stop. A
repoint onto a table that differs in those is exactly the failure this step exists to catch, and
here nothing downstream would fail loudly.
...
- **`warehouse/assets.yaml`** — **add** a new asset for each new table, leaving the two existing
  assets in place (step 5 transitions those to `status: compatibility`; they are removed only at
  step 6). For each new asset:
  - the asset `id` (`signal_github.artifact_state`; `signal_huggingface.artifact_state`);
  - `table` (`currentai.<dataset>.artifact_state`);
  - `files.model` — the new model file added below;
  - `reads` / `read_by` — regenerated by `build/assets.py`, never hand-edited, so the derived
    graph reflects the new files and the repointed SQL;
  - the `mirror:` block — `model_id`, `revision`, `hash`, `local_sha256` and `synced_at` set to
    the values the step-1 redeploy of the new model actually produced. If the platform deploys
    under a new `model_id`, add a `mirror_migration` note so the provenance gate allows the
    `model_id` change (`test_model_id_change_is_allowed_with_a_migration_note`).
- **Model files — add the new ones alongside; do not `git mv` the old ones.** The old mirror
  files stay in place with their `mirror:` blocks untouched for the whole compatibility window,
  because the old tables are still deployed and still in the census. A `git mv` would delete a
  file whose table the platform is still serving, so moving the old file and retaining the old
  table conflict. Add a separately-refetched new file for each new asset instead, with its own
  freshly-fetched bytes and provenance:
  - keep `warehouse/models/signal_github/repo_state.py` (with `def repo_state`) as is, and add
    `warehouse/models/signal_github/artifact_state.py` (with `def artifact_state`);
  - keep `warehouse/models/signal_huggingface/hub_state.py` (with `def hub_state`) as is, and add
...
- **`warehouse/audits/platform_models.json`** — regenerate with
  ...
  hand-edit it; the receipt validator and `test_platform_model_consumers_match_the_receipt` bind
  it to what the audit derives.
- **`docs/architecture/current-state-dag.md`** — regenerate against the current state; the DAG
  gains `signal_github__artifact_state` / `signal_huggingface__artifact_state` nodes and their
  repointed edges alongside the retained old nodes. `test_committed_dag_matches_the_renderer`
  fails if it is left stale.

- **The three repointed consumer models are themselves platform-authored mirrors — advance their
  provenance too.** The consumers repointed in step 3 above are not plain repository SQL; each is
  a platform-authored, mirrored model with its own `mirror:` block. Repointing their `FROM`
  clauses (and, for the evidence adapter, its `source_table` literals) from `repo_state` /
  `hub_state` to the new `artifact_state` tables changes their bytes, so each one's redeployed
  platform revision must be refetched and its `mirror:` block (`revision`, `hash`,
  `local_sha256`, `synced_at`) advanced in this same PR, exactly like the new models above. The
  three:
...
- **Before step 6 (old tables still live).** The flip is reversible with no data loss. Repoint
  ...
  (dropping the new asset ids, the new model files and their mirror blocks, and the repointed
  consumer SQL and its advanced mirrors, and restoring the regenerated census/DAG), and leave the
  new `artifact_state` tables in place or drop them — nothing depends on them once the consumers
  point back. The old files, ids and mirror blocks were preserved untouched throughout, and the
  old tables never stopped serving, so this is a clean revert.
