...
(platform-authored mirror: `model_id cc73d808-5833-4a91-a47b-35bb0414b6f0`, `revision 3`),
`status: active`, `materialized: true`, dataset cron `0 1 * * 0`. Model file
...
registry). It declares `external_origins` and does **not** call `context.fetch()`, so the step-1
twin carries **no `capabilities` delta** — it is byte-identical except for its path/`def`/table.
...
on **code, schema and identity coverage** — the twin's code is byte-identical, so its schema and
resolved-key coverage are identical — and the dynamic-field comparison documents the **shared**
upstream failure rather than treating it as a divergence. Do not gate the move on a clean paired
live run that the upstream will not currently allow; the relocation neither causes nor fixes the
429.
