...
It is a decision register, not an incident log. Every defect that produced the rule has a named
regression test, and git carries the narrative. What a future sweep needs is the other half —
which duplicates are deliberate — because without it the same three constants get rediscovered as
findings every time somebody greps for them.
...
**`render.py` is not exempt from the axes rule.** Being the only build module invoked as a
script would be a reason to exempt it, since a script cannot import from the package, and the
exemption is unsound anyway: a fourth axis would leave the rendered methodology silently omitting
its sources while every other test passed. The invocation is `-m` instead.

**Proposable is not the same set as routed.** Deriving `propose_artifacts`' supported kinds
*from* the routing table fails in the other direction: a newly declared `artifact_key` becomes an
accepted `--kind` with no URL pattern, no live check and no renderer behind it. Support is
defined by the three handlers a kind needs, and asserted to be an intentional subset with `arxiv`
the named gap.

**Visibility is one fact and has five consumers.** A category's lifecycle status decides whether
its products appear in the map, and the payload emits five indexes that each have to honour it:
the categories themselves, `n_total`, the organization rosters, the alias redirects, and the
long-tail sample. A filter applied to three of the five hides a preliminary category's products
from `/map` while leaving them listed under `/map/org/<slug>` and shipping organizations that
exist on the map nowhere else. The long-tail count has the same shape one level up, comparing a
snapshot against every product file while the payload shows only the published ones - and it
fails silently, because a count that is too high looks like a count.
...
Copies of these two diverge in exactly one line — whether a `date` object is accepted — and
PyYAML returns an object for an unquoted date and a string from a payload, so the same freshness
question gets two answers depending on which side of the pipeline the value came from. That is
why both have one owner and every consumer imports.

Its regression test matches on what a function RETURNS, not on its name. A name test passes
`validate_sources` as a false positive because it parses a date inline, and the fix for that must
never be to name the exception: a test that passes by listing whatever it happens to find has
...
**A status or exit derived from fewer conditions than the printed report.** A `--strict` run
that prints a violation, follows it with `[OK]` and exits 0 is the shape. Every checker is held
to the opposite: `check_verification` iterates all its sub-gates so its exit covers everything it
prints, and a line that is deliberately non-gating is marked `~` with a written reason
...
normalizes both - a scalar entry means published. A category contract keyed off which SPELLING was
used rather than off the normalized status buys a published category owing nothing: a scalar entry
pointing at a file with only `name`, `display_name` and an empty roster produces zero errors and
ships visibly empty. The checks run on the normalized value, and the measurement that licenses
that is whether every scalar entry already satisfies them - which is the measurement an exemption
has to make before it is granted, not after.
