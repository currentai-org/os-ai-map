...
def test_a_note_comes_out_per_axis_at_the_sources_grain():
    """The notes table is what lets a consumer read a score's prose without the payload."""
    sources = _sources(
        categories={"base": {"scoring_recipe": RECIPE, "products": ["m"]}},
        scores={
            "m": {
                "openness": {
                    ...
                    "class": "open_weights",
                    "components": "weights:open;data:closed",
                    "note": "Weights are downloadable; the training data is not described.",
                },
                "adoption": {"level": 4, "note": "Top-20 by monthly downloads on PyPI."},
            }
        },
    )
    tables, errors, _ = build_rubric(sources, POLICY, {})
    assert errors == []
    rows = {(r["axis"], r["note"]) for r in tables["product_score_notes"]}
    assert rows == {
        ("openness", "Weights are downloadable; the training data is not described."),
        ("adoption", "Top-20 by monthly downloads on PyPI."),
    }
    for row in tables["product_score_notes"]:
        assert row["product_slug"] == "m"
        assert row["category_slug"] == "base"


@pytest.mark.parametrize("note", [None, "", "   "])
def test_an_axis_with_no_note_gets_no_row(note):
    """Absence rather than an empty string, so "not measured" and "measured, said nothing"
    stay distinguishable in the warehouse. An abstaining axis is the common case."""
    openness = {"score": 3, "class": "open_weights", "components": "weights:open"}
    if note is not None:
        openness["note"] = note
    sources = _sources(
        categories={"base": {"scoring_recipe": RECIPE, "products": ["m"]}},
        scores={"m": {"openness": openness}},
    )
    tables, errors, _ = build_rubric(sources, POLICY, {})
    assert errors == []
    assert tables["product_score_notes"] == []


def test_every_note_the_payload_renders_reaches_the_table(real_rubric):
    """The migration's actual contract: the warehouse can serve the Details modal's prose.

    Both sides are built from the SAME `sources` in this process, deliberately. Comparing the
    table against the COMMITTED `build/notebook_data.json` would be the more obvious test and
    would be wrong: the payload is regenerated by a bot after merge, so between a change to
    sources/scores and that bot commit the two legitimately disagree. build/render.py carries
    the same lag in its `_ORDER` filtering, for the same reason.

    Subset rather than equality, because the table covers every category in sources/ and
    `build_payload` drops preliminary ones. A note in the table with no product on the map is
    fine; a note on the map that the table cannot serve is the failure.
    """
    import json

    from build.serialize import ROOT, build_payload

    sources, tables, errors, _ = real_rubric
    assert errors == []

    frozen = json.loads((ROOT / "sources" / "snapshots" / "long_tail.json").read_text())
    payload = build_payload(sources, frozen, generated="2026-01-01")

    table = {(r["product_slug"], r["category_slug"], r["axis"]): r["note"]
             for r in tables["product_score_notes"]}

    rendered = 0
    for category_slug, category in payload["categories"].items():
        for product in category["products"]:
            for axis in AXES:
                note = ((product.get(axis) or {}).get("note") or "").strip()
                if not note:
                    continue
                rendered += 1
                key = (product["slug"], category_slug, axis)
                assert key in table, f"{key}: the modal renders a note the table does not carry"
                assert table[key] == note, f"{key}: the table's note is not the rendered one"

    assert rendered > 2000, (
        f"only {rendered} notes found in the payload; the corpus carries thousands, so this "
        "test is no longer reaching the prose it is meant to guard"
    )
