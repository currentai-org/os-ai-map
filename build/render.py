...
# The canonical key order, which must match the notebook dict order. It comes from the COMMITTED
# payload while the straplines and weights below come from the live `sources/categories/*.yaml`,
# and those two lag each other by design: the payload is regenerated by a bot after merge, so
# between a change to sources/ and that bot commit the payload names categories sources no longer
# has. Filtering rather than indexing is what keeps this module importable in that window.
#
# It was NOT filtered until 2026-09-17, and the failure mode was bad out of proportion to the
# cause: a renamed category made `cats[cid]` raise KeyError at module scope, which is an import
# error, which takes down the whole of tests/test_render.py as a COLLECTION error rather than a
# readable assertion. A category that is new in sources and not yet in the payload was already
# tolerated - it simply does not render until the bot catches up - and this makes the opposite
# case behave the same way.
_ORDER = [cid for cid in data["order"] if cid in _cats_yaml]
