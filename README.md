...
|-----------|----------|----------|
| `sources/organizations/` | One file per org (`name`=slug, `display_name`, `type`, `homepage`) | Orgs are referenced by slug from product files |
...
| `sources/products/` | One file per product (`name`=slug, `display_name`, `org`, `type`, `description`; typed artifact arrays per source: `github`, `npm`, `pypi`, `crates`, `go`, `huggingface_model`, `huggingface_dataset`; optional `comments`) | One product appears in exactly one category roster |
